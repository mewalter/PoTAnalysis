...
ShowWorkMeans <- function(condition){
  PData <- AllDataTextTib %>% filter(eval(parse(text=condition))) %>%
   ...
   # mutate(Who=str_replace_all(Who,c("2","3","4","5","6"),c("Actual","Department","School","Campus","Preferred")))
    mutate(Who=str_replace_all(Who,c("2","3"),c("Actual","Department")))
...
p + scale_fill_brewer(palette="Blues") +
  labs(title="PoT Hired 2015 and Later (Count = 50)",
       subtitle="Workload Distribution: Expected, Desired, Actual") 
...
current <- c("2","3")
new <- c("Actual","Dept")
mutate(PData, y=str_replace_all(Who,current,new))
...
result <- testdata %>% #mutate(across(x,as.character)) %>% 
  mutate(x=str_replace_all(x,c("1"="d","2"="e","3"="f")))


%>%
  mutate(Who=str_replace_all(Who,c("1","2","3"),"School"))mutate(x=str_replace_all(x,c("2","4"),c("test","test2")))
