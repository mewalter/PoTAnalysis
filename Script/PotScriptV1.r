...
path <- "C:/Users/walterm/Dropbox/Service/APGWorkGroupPoT/Survey/Analysis"
if( .Platform$OS.type == "unix" )
  path <- "~/Dropbox/Service/APGWorkGroupPoT/Survey/Analysis"
setwd(path)

#AllDataNum <- read.csv("Professor of Teaching Effort Distribution and Support Survey_February 24, 2021_21.37_NumChoice.csv",stringsAsFactors=FALSE,
#                    header=TRUE,sep=",")[-c(1,2),]    # load all data
#AllDataTxt <- read.csv("Professor of Teaching Effort Distribution and Support Survey_February 24, 2021_22.13_TextChoice.csv",stringsAsFactors=FALSE,
#                       header=TRUE,sep=",")[-c(1,2),]    # load all data

col_names <- names(read_csv("Professor of Teaching Effort Distribution and Support Survey_February 24, 2021_21.37_NumChoice.csv", n_max = 0))
AllDataNumTib <- read_csv("Professor of Teaching Effort Distribution and Support Survey_February 24, 2021_21.37_NumChoice.csv",
                          col_names = col_names, skip = 3)    # load all data
AllDataTextTib <- read_csv("Professor of Teaching Effort Distribution and Support Survey_February 24, 2021_22.13_TextChoice.csv",
                          col_names = col_names, skip = 3)    # load all data


# if you want to re-order the table by year and then quarter
#AllDataTxt <- AllDataTxt[order(AllDataTxt$Q1.3_2, AllDataTxt$Q1.3_1),]

# bar chart of start-date year with quarter stacking

#AllDataTextTib %>% drop_na(Q1.3_2,Q1.3_1) %>% 
#  ggplot(aes(x=Q1.3_2,fill=Q1.3_1))+
#  geom_bar()
...
pdf(file="YearRank.pdf")
ggplot(data=AllDataTextTib) + geom_bar(mapping=aes(x=Q1.3_2,fill=Q1.2))+
#AllDataTextTib %>% drop_na(Q1.3_2,Q1.2) %>% 
#  ggplot(aes(x=Q1.3_2,fill=Q1.2))+
#  geom_bar()
...
#PlotData_Year_Bar <- AllDataTxt %>% group_by(Q1.3_2) %>% summarise(Freq=n())
#p<-ggplot(data=PlotData_Year_Bar, aes(x=Q1.3_2, y=Freq)) +
#  geom_bar(stat="identity")
#

############ All Work Distribution Data ##################
pdf(file="AllPoT.pdf")
AllAllWorkMeans <- AllDataNumTib %>% summarise(across(16:30, ~ mean(.x, na.rm = TRUE))) %>%
  pivot_longer(
    cols = starts_with("Q2."),
  ...
  mutate(Activity=str_replace_all(Activity, c("1","2","3"),c("Research","Teaching","Service"))) %>%
  mutate(Who=str_replace_all(Who,"2","Actual")) %>%
  mutate(Who=str_replace_all(Who,"3","Department")) %>%
  mutate(Who=str_replace_all(Who,"4","School")) %>% 
  mutate(Who=str_replace_all(Who,"5","Campus")) %>%
  mutate(Who=str_replace_all(Who,"6","Preferred")) 

AllAllWorkMeans$Who <- factor(AllAllWorkMeans$Who,levels = c("Actual", "Preferred", "Department", "School", "Campus"))
ggplot(AllAllWorkMeans, aes(x=Who, y=Distribution, fill=Activity, label=sprintf("%0.1f", round(Distribution, digits = 1)))) +
  geom_bar(stat = "identity")+geom_text(size = 6, position = position_stack(vjust = 0.5)) + 
  scale_fill_brewer(palette="Greens") + coord_flip() +
  labs(title="All PoT (Count = 83)",
       subtitle="Workload Distribution: Expected, Desired, Actual")
dev.off()


############ Greater than 2015 Work Distribution Data ##################
pdf(file="Post2015.pdf")
AllGE2015WorkMeans <- AllDataTextTib %>% filter(Q1.3_2>="2015") %>% 
  summarise(across(16:30, ~ mean(.x, na.rm = TRUE))) %>%
  pivot_longer(
    cols = starts_with("Q2."),
    names_to = c("Who","Activity"),
    names_prefix = "Q2.",
...
AllGE2015WorkMeans$Who <- factor(AllGE2015WorkMeans$Who,levels = c("Actual", "Preferred", "Department", "School", "Campus"))
ggplot(AllGE2015WorkMeans, aes(x=Who, y=Distribution, fill=Activity, label=sprintf("%0.1f", round(Distribution, digits = 1)))) +
  geom_bar(stat = "identity")+geom_text(size = 6, position = position_stack(vjust = 0.5)) + 
  scale_fill_brewer(palette="Blues") + coord_flip()+
  labs(title="PoT Hired Since 2015 (Count = 50)",
       subtitle="Workload Distribution: Expected, Desired, Actual")
dev.off()


############ Less than 2015 Work Distribution Data ##################
pdf(file="Pre2015.pdf")
AllLE2015WorkMeans <- AllDataTextTib %>% filter(Q1.3_2<"2015") %>% 
  summarise(across(16:30, ~ mean(.x, na.rm = TRUE))) %>%
  pivot_longer(
    cols = starts_with("Q2."),
    names_to = c("Who","Activity"),
...
AllLE2015WorkMeans$Who <- factor(AllLE2015WorkMeans$Who,levels = c("Actual", "Preferred", "Department", "School", "Campus"))
ggplot(AllGE2015WorkMeans, aes(x=Who, y=Distribution, fill=Activity, label=sprintf("%0.1f", round(Distribution, digits = 1)))) +
  geom_bar(stat = "identity")+geom_text(size = 6, position = position_stack(vjust = 0.5)) + 
  scale_fill_brewer(palette="Blues") + coord_flip()+
  labs(title="PoT Hired Before 2015 (Count = 33)",
       subtitle="Workload Distribution: Expected, Desired, Actual")
dev.off()



############ Asssistant Work Distribution Data ##################
pdf(file="Assistant.pdf")
AllAssistWorkMeans <- AllDataTextTib %>% filter(Q1.2=="Assistant") %>% 
  summarise(across(16:30, ~ mean(.x, na.rm = TRUE))) %>%
  pivot_longer(
    cols = starts_with("Q2."),
    names_to = c("Who","Activity"),
...
AllAssistWorkMeans$Who <- factor(AllAssistWorkMeans$Who,levels = c("Actual", "Preferred", "Department", "School", "Campus"))
ggplot(AllGE2015WorkMeans, aes(x=Who, y=Distribution, fill=Activity, label=sprintf("%0.1f", round(Distribution, digits = 1)))) +
  geom_bar(stat = "identity")+geom_text(size = 6, position = position_stack(vjust = 0.5)) + 
  scale_fill_brewer(palette="Oranges") + coord_flip()+
  labs(title="Assistant PoTs (Count = 34)",
       subtitle="Workload Distribution: Expected, Desired, Actual")
dev.off()


############ FullAssoc Work Distribution Data ##################
pdf(file="FullAssoc.pdf")
AllFullAssocWorkMeans <- AllDataTextTib %>% filter(Q1.2=="Full"|Q1.2=="Associate") %>% 
  summarise(across(16:30, ~ mean(.x, na.rm = TRUE))) %>%
  pivot_longer(
    cols = starts_with("Q2."),
    names_to = c("Who","Activity"),
...
AllFullAssocWorkMeans$Who <- factor(AllFullAssocWorkMeans$Who,levels = c("Actual", "Preferred", "Department", "School", "Campus"))
ggplot(AllGE2015WorkMeans, aes(x=Who, y=Distribution, fill=Activity, label=sprintf("%0.1f", round(Distribution, digits = 1)))) +
  geom_bar(stat = "identity")+geom_text(size = 6, position = position_stack(vjust = 0.5)) + 
  scale_fill_brewer(palette="Purples") + coord_flip()+
  labs(title="Associate and Full PoTs (Count = 52)",
       subtitle="Workload Distribution: Expected, Desired, Actual")
dev.off()
...
write.csv(AllAllWorkMeans, file = "AllAllWorkMeans.csv",row.names=FALSE)
AllAllWorkMeansT <- read_csv("AllAllWorkMeans.csv")
